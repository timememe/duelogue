...
func _run_clinch(attacker: String, defender: String, idx: int, prefer_steal: bool, named_index: int = -1) -> void:
	...
	# Пол=1 (2026-08-24, баланс-сим sim_emotion_swing.gd): без пола честный stake=0 выпадал
	# 64-65% клинчей у smart/balanced и 36% у aggro — трение технически работало, но было
	# статистически невидимым почти всегда. Пол не двигает ни потолок серьёзности (макс.
	# разовый удар не менялся между вариантами), ни % пирровых побед — только убирает
	# «клинч был, а трения нет». Пол=2 пробовали и отвергли тем же сравнением: он берёт с
	# любого клинча больше, чем стоит база облегчения победителя (=1), — 83% пирровых побед
	# у aggro в том же прогоне, хуже даже варианта без пола делить на 1 вовсе.
	var clinch_length := r_count + t_added
	var stake := maxi(1, clinch_length / 2)
