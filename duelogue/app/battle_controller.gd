...
## Сколько ещё доигрывает текущая катсцена/вспышка/баннер, секунд (0 — уже всё отыграло).
## Публичный двойник _pace_left() — для внешних потребителей, которым мало факта, что
## choose_target() вернулся: _run_clinch не флешит СВОЙ хвостовой _say_until перед возвратом
## (вспышка исхода/баннер комбо могут остаться недоигранными на момент возврата — это его
## собственный контракт с _say/_wait_pace, рвать его отдельным await внутри самого
## _run_clinch ради одного внешнего вызова не стали). Кто нуждается в «презентация ПОЛНОСТЬЮ
## закончилась», а не просто «ход решён» — например debate_screen._resolve_targeted_land
## перед посадкой карты-призрака — ждёт это ЗДЕСЬ, явно, а не молчаливо предполагает.
func remaining_pace() -> float:
	return _pace_left()
