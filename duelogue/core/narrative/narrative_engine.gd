...
## Общая риторическая онтология (схемы/масти/зацепки) переехала в grammar.gd и читается
## оттуда одним источником (контракт combo_grammar_v0.2 §12). Grammar — чистые данные без
## зависимостей; развязка от ядра ПРАВИЛ сохранена.
const Grammar := preload("res://duelogue/core/cards/grammar.gd")

# Имя карты модели → приём (манера). Имена из zal_v3_model.gd / deck.gd.
# FALLBACK для архивных данных и старых сейвов: авторитет — поля scheme/device на карте.
const TEZIS_DEV := {
...
const DEV_APPEAL := Grammar.SUIT_OF
...
## Приём карты. Авторитет — поля самого объекта (scheme/device, тегируются фабрикой deck
## по grammar.gd); name-мапы — только fallback для архивных карт без полей.
func device_for(card: Dictionary) -> String:
	match String(card.get("type", "")):
		TYPE_USTANOVKA: return "Установка"
		TYPE_TEZIS:
			var scheme := String(card.get("scheme", ""))
			if scheme != "":
				return scheme
			return TEZIS_DEV.get(String(card.get("name", "")), "Пример")
		TYPE_RAZBOR:
			if bool(card.get("steals", false)):
				var d: String = KRAJA_DEV[_kraja_i % KRAJA_DEV.size()]
				_kraja_i += 1
				return d
			var device := String(card.get("device", ""))
			if device != "":
				return device
