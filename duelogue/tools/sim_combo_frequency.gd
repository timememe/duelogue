...
## def_will_clinch/atk_will_clinch у всех стилей решают ТОЛЬКО «играть/пасовать»; какую
## именно карту защиты выбрать — clinch_submit(hand_index=-1) внутри ядра, без учёта
## стиля (def_answer_index есть в ai.gd, но auto-resolve сима его не зовёт). Поэтому
## разница между стилями ниже отражает в основном СТРУКТУРНУЮ вероятность (кто во что
## бьёт), а не разницу «мешер vs эксперт» — сознательного combo-поиска в сime пока нет.
const PAIRINGS := [
	["balanced", "balanced"],
	["aggro", "aggro"],
	["smart", "smart"],
