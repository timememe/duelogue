...
## Запуск: godot --headless --script res://test/prototype_zal/narrative_smoke.gd
...
const TX_PATH := "res://test/prototype_zal/narrative_transcript_smoke.md"
...
func _play_match(seed: int, theme: Dictionary) -> void:
	...
	while not model.game_over and guard < 300:
		...
		if st == "pass":
			...
			model.advance(); continue
		var act: Dictionary = model.ai_pick(side, "balanced")
		if act.is_empty():
...
func _auto_clinch(attacker: String, defender: String, idx: int) -> void:
	...
	while guard < 40:
		guard += 1
		if model.has_card(defender, ZalV3.TYPE_TEZIS) and model.ai_def_will_clinch(defender, line):
			...
			break
		if model.has_card(attacker, ZalV3.TYPE_RAZBOR) and model.ai_atk_will_clinch(attacker, line):
